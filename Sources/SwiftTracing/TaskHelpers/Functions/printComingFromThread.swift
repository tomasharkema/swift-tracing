...
//
  ...
  public func _printComingFromThread<ReturnType>(
    allowFromMainThread: Bool = false,
    @_implicitSelfCapture @_inheritActorContext _ handler:
      @Sendable @escaping () async
      -> ReturnType,
    _ file: StaticString = #file, _ line: UInt = #line, _ function: String = #function
  ) async -> ReturnType {
  ...
  @_transparent
  @inline(__always)
  func innerDebugTaskCall<ReturnType: Sendable>(
    caller: Caller,
    priority: TaskPriority?,
    @_implicitSelfCapture @_inheritActorContext _ handler:
      @Sendable @escaping () async -> ReturnType
  ) {
